  ...
  <div class="consumptionJinBox">
    <div class="balanceHead" :style="balanceHeadStyle">
      <div class="">
        ...
        <div class="" style="font-size: .75rem;padding:1.625rem 0rem 0rem 1.625rem;color: rgb(255,255,255);">
          注：消费使用时，不足1个消费金将按1个消费金计算
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    computed:{
      balanceHeadStyle(){
        return {backgroundImage: imgUrl? 'url('+imgUrl+'bag_1.png)' : ''}
      }
    },
    ...
    created() {
      ...
      this.consumeBalance = option.consumeBalance
      this.getInfo()
    },
    methods: {
      async getInfo() {
        this.status = 'loading';
        let res = await this.$http.post('/api/v1/ConsumeRecord/GetList', {
