<template>
  <!--地址picker-->
  <div :status="checkStatus" v-if="lotusAddressData.visible" class="lotus-address-mask">
    <div :class="lotusAddressData.visible?'lotus-address-box':'lotus-address-box lotus-address-box-out'">
      <div class="lotus-address-action">
        <span @click="cancelPicker" class="lotus-address-action-cancel">取消</span>
        <span @click="chosedVal" class="lotus-address-action-affirm">确认</span>
      </div>
      <div class="lotus-address-picker-box">
        <!--省-->
        <list class="lotus-address-picker-box-item">
          <div @click="clickPicker(0,pIndex,pItem);" :id="'pid'+pIndex"
            :class="pIndex === pChoseIndex?'lotus-address-picker lotus-address-picker2':'lotus-address-picker'"
            v-for="(pItem,pIndex) in province" :key="pIndex">{{pItem}}</div>
        </list>
        <!--市-->
        <list class="lotus-address-picker-box-item">
          <div @click="clickPicker(1,cIndex,cItem);" :id="'cid'+cIndex"
            :class="cIndex === cChoseIndex?'lotus-address-picker lotus-address-picker2':'lotus-address-picker'"
            v-for="(cItem,cIndex) in city" :key="cIndex">{{cItem}}</div>
        </list>
        <!--区-->
        <list class="lotus-address-picker-box-item">
          <div @click="clickPicker(2,tIndex,tItem);" :id="'tid'+tIndex"
            :class="tIndex === tChoseIndex?'lotus-address-picker lotus-address-picker2':'lotus-address-picker'"
            v-for="(tItem,tIndex) in town" :key="tIndex">{{tItem}}</div>
        </list>
        <!--区END-->
      </div>
    </div>
